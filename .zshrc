...
precmd_vcs_info() { 
  ...
  if [[ -n ${vcs_info_msg_0_} ]]; then 
    ...
    PS1="${WHITE}user@host %2~ ${BOLD_BLUE}( ${vcs_info_msg_0_} ${BOLD_BLUE}) ${GREEN}%% " 
  else 
    #no messages from vcs, show longer pwd 
    PS1="${WHITE}user@host %5~ ${GREEN}%% "
  fi 
} 
...
function +vi-git-untracked(){
  ...
  if [[ $(git rev-parse --is-inside-work-tree 2> /dev/null) == 'true' ]] && \
        git status --porcelain | grep ' M ' &> /dev/null || git status --porcelain | grep ' D ' &> /dev/null ; then
     stagedchange=' | *' 
  fi
...
function +vi-git-st-staged(){ 
  if [[ $(git rev-parse --is-inside-work-tree 2> /dev/null) == 'true' ]] && \
        git status --porcelain | grep 'M  ' &> /dev/null || git status --porcelain | grep 'A  ' &> /dev/null || git status --porcelain | grep 'D  ' &> /dev/null ; then 
    hook_com[misc]=${BOLD_CYAN}' | *' 
  fi 
}

#define function differences between local and remote
...
#tell vcs_info how to look for git repos
